...
def hev_remanufacture_sensitivity(engine, st):
    """Perfil híbrido: quanto pode custar remanufaturar um pack antes de a reciclagem voltar a vencer.

    O custo de remanufatura de um pack de 1,3 kWh não tem fonte pública, então NÃO é assumido: é varrido.
    `breakeven_brl_pack` = custo de remanufatura por pack no qual VPL(remanufatura) = VPL(reciclagem).
    """
    rows = []
...
def main():
    ...
    for prof in PROFILES:
        ...
        if prof == "hev_nimh_corolla":
            node["sensibilidade_remanufatura"] = {lm: hev_remanufacture_sensitivity(engine, profile_settings(prof, life_model=lm))
                                                  for lm in SCENARIOS}
